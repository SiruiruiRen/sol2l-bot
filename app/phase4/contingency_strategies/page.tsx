...
export default function ContingencyPlanPage() {
  ...
  const handleComplete = () => {
    ...
    if (!completedTasks.includes('contingency_strategies')) {
      completedTasks.push('contingency_strategies');
    ...
    >
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <Card className="bg-[rgba(24,17,28,0.78)] backdrop-blur-md border shadow-xl mb-6" style={{ borderColor: phaseColor.cardBorder }}>
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center justify-center gap-3 text-2xl md:text-3xl font-bold text-center">
                <ShieldCheck className="h-8 w-8" style={{ color: phaseColor.accent }} />
          ...
          {/* Centered Proceed to Phase 5 Button */}
          {isComplete && (
            <div className="flex justify-center mt-6">
              ...
              >
                Proceed to Phase 5 <ChevronRight className="h-4 w-4 ml-2" />
              </Button>
            </div>
          )}
        </motion.div>
